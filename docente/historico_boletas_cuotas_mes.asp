...
<%  Dim o, n_NrRutProfe, n_NrAnoActivo, n_NrPerActivo, s_NmBrsProfe, d_FcSistema, n_TotalPagoSemestre, s_CdCodigo, s_NmMesF, n_NrMesF, s_NmUnidadAcadF, n_CdUnidadAcadF
	...
	Session.TimeOut = 10		
	Set o = getComponent()
	s_CdCodigo = request("s_codigo")
	If s_CdCodigo <> "" then 
	   s_NmMesF = CStr(Separa2(s_CdCodigo))
	   n_NrMesF = CInt(Separa2(s_CdCodigo))
	   s_NmUnidadAcadF = CStr(Separa2(s_CdCodigo))
	   n_CdUnidadAcadF = CInt(Separa2(s_CdCodigo))
	   s_TpConsultaF = CStr(Separa2(s_CdCodigo))
	End If
	d_fecha = date()
	If d_fecha <> "" Then
...
<% call response.write (s_NmBrsProfe) %>
        ...
        <tr>
          <td valign="top" class="txsubtitulo">Unidad Academica de <strong>
          <% call response.write (s_NmUnidadAcadF) %>
          </strong></td>
        </tr>
        <tr>
          <% Call o.Consulta_Nombre_Mes(s_NmMes) %>
          <td colspan="2" valign="top" class="txsubtitulo">Detalle mes de <strong>
            <% call response.write (s_NmMesF) %>
          </strong></span></td>
        </tr>
		<tr>
        ...
        <tr>
          <td valign="top"><table width="560" border="1" cellpadding="1" cellspacing="0" bordercolor="#999999">
			...
			<% 'n_NrRutProfe = 3398351
				     'n_AnoSistema = 2009
					 'n_NrPerActivo = 1
                         xml = o.Consulta_Asignaturas_Cuota_Mes(n_NrRutProfe, n_NrAnoActivo, n_NrPerActivo, n_NrMesF, n_CdUnidadAcadF, s_TpConsultaF) 
              ...
              <td width="156"><div align="center"><strong>Unidad Acad&eacute;mica</strong></div></td>   
			  <% If s_TpConsultaF = "TI"  Then %>
			    <td width="121"><strong>Proyecto</strong></td>	
			    <td width="121"><strong>Funci&oacute;n</strong></td>				
			     <% Else %>
                 <td width="121"><strong>Carrera</strong></td>
                 <td width="144"><strong>Asignatura</strong></td>
                 <td width="25"><strong>Act</strong>.</td>
                 <td width="31"><strong>Par</strong>.</td>
			   <% End If%>
              <td width="57"><div align="center"><strong>Monto Bruto $</strong></div></td>
            </tr>
            <%							  
            ...
            <tr class="Estilo4">
			  ...
			  <% If s_TpConsultaF = "TI"  Then %>
			     <td><span class="Estilo3" ><font face="Arial, Helvetica, sans-serif">
                 <%response.write (valor(LIST_CARGA_DOCENTE_M,"s_NmCarreraM"))%>
                 </font></span></td>
				 <td><span class="Estilo3" ><font face="Arial, Helvetica, sans-serif">
                 <%response.write (valor(LIST_CARGA_DOCENTE_M,"s_NmAsignaM"))%>
                 </font></span></td>
				 <td><div align="right"><span class="Estilo3" ><font face="Arial, Helvetica, sans-serif">
